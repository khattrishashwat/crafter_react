...
function Additional() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (searchClient) {
        ...
        setIsLoading(true);
        response = await axios.get(`workmen/checklist/`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const currentPosts = checkList.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(
    (Array.isArray(checkList) ? checkList.length : 0) / postsPerPage
  );
      ...
      <div className="big-containe bg-1">
        <div className="small-container">
          <div className="same-as-head">
            <ul>
              <li>
                <Link to="/additional" className="actives">
                  Checklists
                </Link>
              </li>
              <li>
