  ...
  useEffect(() => {
    const fetchObjects = async () => {
      try {
        ...
        setObjects(response.data.data.results || []);
      } catch (error) {
        console.error("Error fetching objects:", error);
      } finally {
        setIsLoading(false); // Set loading to false after fetching data (whether success or error)
      }
    };
        ...
        <div className="work-flex">
          ...
          {/* <div className="fields">
            <div className="input-1">
              <input type="text" placeholder="Looking for a client" />
            </div>
            <div className="fields">
              <div className="input-1">
                <input type="text" placeholder="Find work location" />
              </div>
              <div className="fields">
                <div className="input-1">
      ...
      <div>
        {isLoading ? (
          <div className="loader-container">
            ...
            {/* You can customize the loader style as per your UI/UX */}
          </div>
        ) : (
                              ...
                              {objects.length === 0 ? (
                                ...
                                <tbody>
                                  {objects.map((obj, index) =>
                                    obj.object[0] ? (
                                      <tr key={index}>
                                        <td>{obj.object[0].name}</td>
                                        <td>{obj.object[0].remark}</td>
                                        <td>{obj.object[0].type}</td>
                                      </tr>
                                    ) : null
                                  )}
                                </tbody>
